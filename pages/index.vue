  ...
  <v-row justify="center" align="center">
    <v-col cols="12" sm="12" md="12">
      ...
      <v-sheet class="mx-auto mb-4" elevation="8">
        ...
        <v-slide-group show-arrows class="px-4 d-none d-sm-flex">
          <v-slide-item v-for="article in articles['content']" :key="article['id']">
            <v-card class="ma-2" width="300px">
              <honor-user-bar :user="article.user" :date="article.createTime" small simplify />
        ...
        <div v-for="article in articles['content']" :key="article['id']" class="px-4 d-sm-none">
          <v-card class="ma-2">
            <honor-user-bar :user="article.user" :date="article.createTime" small simplify />
            <v-divider class="mx-4"></v-divider>
            <v-card-title class="subtitle-1">
      ...
      <v-sheet class="mx-auto mb-4" elevation="8">
        ...
        <v-slide-group show-arrows class="px-4 pb-4 d-none d-sm-flex">
          <v-slide-item v-for="article in notes['content']" :key="article['id']">
        ...
        <div v-for="article in notes['content']" :key="article['id']" class="px-4 d-sm-none">
          <v-card class="ma-2">
            <honor-user-bar :user="article.user" :date="article.createTime" small simplify />
            <v-divider class="mx-4"></v-divider>
            <v-card-title class="subtitle-1">
      ...
      <!-- 共同体开始 -->
      <v-sheet v-if="communities.content.length > 0" class="mx-auto mb-4" elevation="8">
        <v-card-title class="ml-4 text-h5 pb-0">共同体</v-card-title>
        <v-slide-group show-arrows class="px-4 pb-4 d-none d-sm-flex">
          <v-slide-item v-for="article in communities['content']" :key="article['id']">
            <v-card class="ma-4" width="400px">
              <v-img v-if="article.img" :lazy-src="article['img'].base64" :src="article['img'].original.url"
                class="mx-auto" height="150" :srcset="getSrcSet(article['img'])" contain
              ...
              <v-img v-else class="mx-auto" height="150" src="/not-found.png" contain sizes="(min-width: 1280px) 1280px">
                <template #placeholder>
                  <v-row class="fill-height ma-0" align="center" justify="center">
                    <v-progress-circular indeterminate color="grey lighten-5"></v-progress-circular>
                  </v-row>
              ...
              <v-list-item style="height: 100px">
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle>{{ article['describe'] }}
                  </v-list-item-subtitle>
                </v-list-item-content>
              </v-list-item>
              <v-divider class="mx-4"></v-divider>
              <v-card-actions class="justify-space-between mx-2">
                <honor-user-bar :user="article.user" :date="article.createDate" small simplify></honor-user-bar>
        ...
        <div v-for="article in communities['content']" :key="article['id']" class="px-4 d-sm-none">
          <v-card class="mb-2" @click="click_community(article['id'])">
            <v-img v-if="article.img" :lazy-src="article['img'].base64" :src="article['img'].original.url" class="mx-auto"
              height="150" :srcset="getSrcSet(article['img'])" contain sizes="(min-width: 1280px) 1280px">
              <template #placeholder>
                <v-row class="fill-height ma-0" align="center" justify="center">
                  <v-progress-circular indeterminate color="grey lighten-5"></v-progress-circular>
                </v-row>
